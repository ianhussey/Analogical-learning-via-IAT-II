...
# get data
data_df <- 
  read.csv("data/processed/wide all tasks data.csv") %>%
...
AMP_data <- 
  read.csv("data/processed/long AMP data.csv") %>%
...
```{r, message=FALSE, warning=FALSE, echo=FALSE}
ratings_data <- 
  read.csv("data/processed/long ratings data.csv") %>%
...
sjt.lmer(model_2, 
         emph.p = FALSE, 
         #p.kr = TRUE,  # doesn't seem to be working
         show.std = TRUE)
```
